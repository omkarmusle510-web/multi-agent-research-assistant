...
class FinalReport(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        if not self.title and self.topic:
            self.title = f"Research Briefing: {self.topic}"
    ...
    def to_markdown(self) -> str:
        """Render a concise, evidence-first research briefing as GitHub Flavored Markdown."""
        lines: List[str] = []
        topic_title = self.topic or self.title or "Research Briefing"
        lines.append(f"# Research Briefing: {topic_title}")
        ...
        lines.append("## Where the Evidence Conflicts\n")
        if self.conflict_analysis:
            for c in self.conflict_analysis:
                lines.append(f"### {c.topic}")
                side_a_ids = ", ".join(c.side_a_sources) if c.side_a_sources else "—"
                side_b_ids = ", ".join(c.side_b_sources) if c.side_b_sources else "—"
                lines.append(f"- **{side_a_ids}** -> {c.side_a}")
                lines.append(f"- **{side_b_ids}** -> {c.side_b}")
                if c.resolution_or_assessment:
                    lines.append(f"- *Why they differ*: {c.resolution_or_assessment}")
                lines.append("")
        else:
            lines.append("No meaningful conflicts identified across the analyzed sources.\n")
